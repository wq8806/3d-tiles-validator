/**
 * sparse-octree v6.1.0 build Thu Sep 03 2020
...
class PointOctant extends Octant {
	...
	getBlancePoint(){
		...
		for (let i = 0; i < points.length; i++) {
			...
			zArray.push(points[i].z);
		}
		xArray = xArray.sort(function(a, b){return a - b});
		yArray = yArray.sort(function(a, b){return a - b});
		zArray = zArray.sort(function(a, b){return a - b});

		const length = points.length;
		const x = xArray[Math.floor(length / 2 )];
		const y = yArray[Math.floor(length / 2 )];
		const z = zArray[Math.floor(length / 2 )];

		return new Vector3(x,y,z);
	}

	distanceToMinSquared(point) {

		const min = this.min;

		const dx = point.x - min.x;
		const dy = point.y - min.y;
		const dz = point.z - min.z;
...
function insert(point, data, octree, octant, depth) {
	...
	if(octant.contains(point, octree.bias)) {
		...
		if(children === null) {
			...
			if(octant.points.length < octree.maxPoints || depth === octree.maxDepth) {
...
class PointOctree extends Octree {
	...
	/**
	 * set all points in this octree.
	 *
	 * @param {Array<Vector3>} points - all points.
	 * @param {Array} datas - all datas.
	 */
	setAllPoints(points,datas){
		this.root._allPoints = points;
		this.pointCount = points.length;
		// this.root.splitByAllPoints();
		this.root.split();
		this.root._allDatas = datas;
		this.redistributeToOctant();
		// this.root.redistribute(this.bias);
	...
	redistributeToOctant(){
		const octant0 = new Vector3(this.min.x,this.min.y,this.min.z);
		const octant1 = new Vector3(this.min.x,this.min.y,this.max.z);
		const octant2 = new Vector3(this.min.x,this.max.y,this.min.z);
		const octant3 = new Vector3(this.min.x,this.max.y,this.max.z);

		const octant4 = new Vector3(this.max.x,this.min.y,this.min.z);
		const octant5 = new Vector3(this.max.x,this.min.y,this.max.z);
		const octant6 = new Vector3(this.max.x,this.max.y,this.min.z);
		const octant7 = new Vector3(this.max.x,this.max.y,this.max.z);

		const corners = [octant0,octant1,octant2,octant3,octant4,octant5,octant6,octant7];
		const points = this.root._allPoints;
		const datas = this.root._allDatas;
		const map = new Map();

		for (let i = 0; i < points.length; i++) {
			let toInsert = new Map();
			for (let j = 0; j < this.root.children.length; j++) {
				const child = this.root.children[j];
				if(child.points === null || child.points.length < this.maxPoints){
					toInsert.set(j,corners[j]);
				}
			}
			map.clear();
			toInsert.forEach(function (value, key) {
				const distance = distanceSquared(points[i],value);
				map.set(key,distance);
			});

			const mapArray = Array.from(map);
			mapArray.sort(function(a,b){return a[1]-b[1]});
			const index = mapArray[0][0];
			if(this.root.children[index].points === null){
				this.root.children[index].points = [];
				this.root.children[index].data = [];
			}
			this.root.children[index].points.push(points[i]);
			this.root.children[index].data.push(datas[i]);
		}
		delete this.root._allPoints;
		delete this.root._allDatas;
	}
